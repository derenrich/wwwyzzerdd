...
const wikidataLinkRegex = new RegExp(
  "^https?://(([a-z]+).)?(?:m.)?wikidata.org/wiki/(Q\\d+)$",
  "i"
);
const wikiExpandedRegex = new RegExp(
  "^https?://[a-z]+.(?:m.)?(wikipedia|wikidata).org.+",
  "i"
);
...
function operateLinks(fn: (link: HTMLAnchorElement) => void) {
  ...
  for (const link of bodyLinkElms) {
    ...
    for (const navBox of navBoxes) {
      if (
        (navBox.compareDocumentPosition(link) &
...
function isWikidataLink(link: HTMLAnchorElement): boolean {
  ...
  return !!(
    link.href &&
    !hrefString.startsWith("#") &&
    wikidataLinkRegex.exec(link.href)
  );
}
...
const decimalCoordRegex = new RegExp(
  ".*//geohack.toolforge.org/geohack.php\\?(?:pagename=(?:.*)&)?params=([\\d.-]+_[NS])_([\\d.-]+_[WE])",
  "i"
);
const secondsCoordRegex = new RegExp(
  ".*//geohack.toolforge.org/geohack.php\\?(?:pagename=(?:.*)&)?params=(\\d+)_(\\d+)_([\\d.]+)_([NS])_(\\d+)_(\\d+)_([\\d.]+)_([WE])",
  "i"
);
...
function boot() {
  ...
  const wikiNamespace =
    document.getElementsByTagName("body")[0].getAttribute("mw-ns") || "";
  const wikiUserLang =
    document.getElementsByTagName("body")[0].getAttribute("mw-lang") || "";
  const pageId = parseInt(
    document.getElementsByTagName("body")[0].getAttribute("mw-page-id") || ""
  );
  const isPage =
    document.getElementsByTagName("body")[0].getAttribute("mw-is-page") ===
    "true";
  const pageName = (
    document.getElementsByTagName("body")[0].getAttribute("mw-page-name") || ""
  ).replaceAll("_", " ");
  ...
  if (isPage && wikiNamespace == "0" && pageName !== "Main Page") {
    ...
    function setRef(ref: WwwyzzerddHolder) {
      ...
      let titleBox: HTMLHeadingElement | null =
          ...
          ref.broker.sendFrontendRequest(
            ...
            (resp: Message) => {
              ...
              if (payload.match) {
                ref.addExternalLink(
                  payload.match.prop,
                  payload.match.identifier,
                  linkAnchor,
                  payload.match.caseInsensitive
                );
              }
            }
          );
